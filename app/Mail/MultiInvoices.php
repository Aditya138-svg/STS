...
class MultiInvoices extends Mailable
{
     ...
     * @return void
     */
    public function __construct($toEmail,$content, $report_invoice_pdf, $cc_emails = [], $bcc_emails = [])
    {
        $this->toEmail = $toEmail;
        $this->content = $content;
        $this->report_invoice_pdf = $report_invoice_pdf;
    ...
    {
        $subj = env('APP_NAME', 'Specialized Transport Systems') . ' Accounting Reminder Notification - ' . date('m/d/Y');
        $email = $this->subject($subj)
					->markdown('emails.multiple_invoices')
                    // ->view('emails.report_invoice')
                    ->with($this->content)
                    ->attachData($this->report_invoice_pdf,'OpenOrOverDueInvoices.pdf', [
