...
class SendPostDeliveryNotification extends Mailable
{
    ...
    {
        $subj = trans('custom.post_delivery_email_sub', ['s' => $this->content['orders_id']]);
        if(!empty($this->content['d_address_type'])){
            $subj = str_replace("DELIVERED!","DELIVERED (".$this->content['d_address_type'].") !", $subj);
        }

        $message = $this->subject($subj)
					->view('emails.post_delivery_notification')	
					->with($this->content);
