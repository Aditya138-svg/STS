...
class SendPaymentReceipt extends Mailable
{
     ...
     * @return void
     */
    public function __construct($toEmail,$content, $payment_receipt_pdf, $cc_emails = [], $bcc_emails = [])
    {
        $this->toEmail = $toEmail;
        $this->content = $content;
        $this->payment_receipt_pdf = $payment_receipt_pdf;
    ...
    public function build()
    {
        ...
        $subj = trans('custom.payment_receipt_email_sub', ['s' => $this->content['orders_id']]);
        $email = $this->subject($subj)
					// ->markdown('emails.schedule_notification')
                    ->view('emails.payment_receipt')
                    ->with($this->content)
                    ->attachData($this->payment_receipt_pdf,'payment_receipt.pdf', [
