...
class SendAutoScheduleNotification extends Mailable
{
    ...
    {
        $subj = trans('custom.auto_schedule_email_sub', ['s' => $this->content['orders_id']]);
        $email = $this->subject($subj)
					// ->markdown('emails.schedule_notification')
                    ->view('emails.auto_schedule_notification') 
					->with($this->content);
