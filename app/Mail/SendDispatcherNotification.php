...
class SendDispatcherNotification extends Mailable
{
    ...
    {
        $subj = trans('custom.dispatcher_notification_sub', ['s' => $this->content['orders_id']]);
        $email = $this->subject($subj)
					// ->markdown('emails.schedule_notification')
                    ->view('emails.call_ahead_notification') 
					->with($this->content);
